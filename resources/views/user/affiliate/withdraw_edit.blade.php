...
<hr>
<form class="ajax reset" action="{{ route('user.affiliate.request.status.change') }}"
    ...
    <div class="col-12 ">
        <div class="zForm-wrap ">
            ...
            <select class="sf-select-without-search cs-select-form" id="product-id" name="status">
                <option {{ $withdraw->status == STATUS_ACTIVE ? 'selected' : ''}} value="{{ STATUS_ACTIVE}}">{{__('Paid')}}</option>
                <option {{ $withdraw->status == STATUS_CANCELED? 'selected' : ''}} value="{{ STATUS_CANCELED }}">{{__('Reject')}}</option>
            </select>
        </div>
    </div>
